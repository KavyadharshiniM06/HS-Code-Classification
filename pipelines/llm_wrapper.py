...
class GroqCleaner:
    ...
    # FIX: patterns that indicate the LLM returned a prefix artifact or
    # echoed back the noisy input rather than cleaning it
    GARBAGE_OUTPUT_PATTERNS = [
        r"^\s*[a-z0-9]\s*[-–]\s+",       # single char prefix like "v - " or "j - "
        r"^\s*[a-z0-9]{1,2}\s+[-–]",      # "56 -" style prefixes
        r"nec\s+in\s+head",               # echoed HS boilerplate
        r"\bheading\s+no\b",
        r"\bitem\s+no\b",
        r"\bsubheading\b",
        r"\bchapter\s+\d",
        r"^\s*\w{1,2}\s*$",               # single/double char result
    ]
    ...
    def clean(self, raw_text: str) -> str:
        """
        Normalize a receipt/invoice line into a short product phrase.
        Returns empty string for non-product lines.
        """
        normalized = self._normalize_text(raw_text)
        if not normalized or self._is_non_product_line(normalized):
            return ""

        prompt = f"""You normalize noisy invoice lines for HS-code retrieval.

Task:
Convert the input into one short lowercase product description.

Rules:
- Output only the cleaned product text (2-6 words ideal)
- Do not add labels like "product description:" or "cleaned:"
- Do not explain your reasoning
- Remove prices, totals, invoice ids, tax ids, store messages, addresses, company names
- Remove HS code numbers, heading references, subheading references
- Remove obvious brand or marketing words when they are not the product itself
- Keep the core merchandise meaning (e.g. "cotton fabric", "diesel engine parts")
...
"""
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Return only a short cleaned product phrase (2-6 words) or skip.",
                    },
            ...
            if cleaned and self._is_plausible_product(cleaned):
                return cleaned
    ...
    def clean_lines(self, raw_invoice: str) -> list:
        ...
        # FIX: flush any remaining buffer after the loop (was inside loop before)
        if buffer:
            cleaned_lines.append({
                "text": buffer,
                "confidence": self.confidence(buffer),
            })
    ...
    def _is_non_product_line(self, text: str) -> bool:
        ...
        product_hits = sum(kw in lower for kw in self.PRODUCT_KEYWORDS)
    ...
    def _is_plausible_product(self, text: str) -> bool:
        """
        FIX: reject outputs that are obviously garbage — single chars, HS
        boilerplate echoed back, or prefix artifacts from noisy inputs.
        """
        if not text:
            return False
        ...
        for pattern in self.GARBAGE_OUTPUT_PATTERNS:
            if re.search(pattern, lower):
                return False

        # Must have at least one alpha word longer than 2 chars
    ...
    def _postprocess_output(self, text: str) -> str:
        ...
        cleaned = re.sub(r"^output\s*:\s*", "", cleaned)
    ...
    def _local_clean(self, text: str) -> str:
        ...
        stopwords = {
            "mrp", "gst", "tax", "card", "document", "discount",
            "thank", "you", "please", "again", "customer", "service",
            "hotline", "feedback", "download", "google", "play",
            "app", "store", "licensee", "goods", "sold", "return",
            "returnap", "retur", "bangunan", "level", "tel", "johor",
            "selangor", "jalan", "kawasan", "perindustrian", "seri",
            "sdn", "bhd",
            # FIX: also strip common HS description boilerplate that leaks in
            "nec", "heading", "subheading", "chapter", "excluding",
            "including", "whether",
        }
