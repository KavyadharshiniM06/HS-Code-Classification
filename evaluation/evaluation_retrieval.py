...
from evaluation.evaluation_dataset import create_synthetic_dataset
...
def evaluate(retriever, dataset, top_k=5):
    ...
    for sample in dataset:
        query = sample["query"]
        true_code = sample["true_hs_code"]

        if hasattr(retriever, "search"):
            results = retriever.search(query, top_k=top_k)
            ranked_codes = [code for code, _ in results]

        elif hasattr(retriever, "retrieve"):
            results = retriever.retrieve(query, top_k=top_k)
            ranked_codes = [r["hs_code"] for r in results]
